# TinyML Shadow PoC — H0：振動頻譜可分性(已推進至 H1,見下方說明)

> **⚠ 2026-08-22 起已推進到 H1，2026-09-07 已完成到 H3**:題目從「D3 車 + TT 馬達振動可分性」收斂成獨立 MeArm 夾爪的過載卡料偵測(分類，已完成並上韌體即時判斷+自動安全 release)+電流限制式自適應夾持(迴歸，規劃中，尚未開始)。H0(本文件描述的振動 PoC)是驗證方法論的第一步、已完成且保留作歷史記錄。韌體見 [`firmware/eep_gripper_h1/`](firmware/eep_gripper_h1)、即時儀表板見 [`live-dashboard/`](live-dashboard)、訓練工具見 [`tools/train_h3_classifier.py`](tools/train_h3_classifier.py)，快速上手看 [`HANDOFF.md`](HANDOFF.md),完整規劃與決策脈絡見權威來源 [`WARROOM-2.0-PLAN.md`](WARROOM-2.0-PLAN.md)。本 repo 已於 2026-09-01 把這條硬體加碼線合併進 `dev`/`main`,跟 `src/` 的離線 SPA 並存展示,不是取代關係。

這個資料夾是「AI 生產異常戰情室」的個人硬體加碼展示，H0 階段用來驗證 TT 馬達在**正常**與**負載**兩種狀態下的振動頻譜是否有肉眼可辨的差異。這條線不會修改、匯入或影響專題主線的離線 SPA（`src/`）；正式評分交付仍以 SPA 為準，硬體是加碼展示。
