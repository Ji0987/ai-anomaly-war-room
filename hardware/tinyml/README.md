...
> **⚠ 2026-08-22 起已推進到 H1**:題目從「D3 車 + TT 馬達振動可分性」收斂成獨立 MeArm 夾爪的電流限制式自適應夾持(迴歸)+ 過載卡料偵測(分類)。H0(本文件描述的振動 PoC)是驗證方法論的第一步、已完成且保留作歷史記錄,**目前實際開發中的是 H1**:韌體見 [`firmware/eep_gripper_h1/`](firmware/eep_gripper_h1)、即時儀表板見 [`live-dashboard/`](live-dashboard),快速上手看 [`HANDOFF.md`](HANDOFF.md),完整規劃與決策脈絡見權威來源 [`WARROOM-2.0-PLAN.md`](WARROOM-2.0-PLAN.md)。本 repo 已於 2026-09-01 把這條硬體加碼線合併進 `dev`/`main`,跟 `src/` 的離線 SPA 並存展示,不是取代關係。
