...
> **⚠ 2026-08-22 第七輪 pivot 後本文件的硬體/題目描述已過時**：D3 車版本（機械手臂裝在四輪車體上）已被取代，使用者拆下 MeArm、放棄 D3 車體與 PS2，題目收斂成獨立夾爪致動器的自適應夾持＋過載偵測。權威規劃見 [`WARROOM-2.0-PLAN.md`](WARROOM-2.0-PLAN.md)，快速上手見 [`HANDOFF.md`](HANDOFF.md)。以下內容保留作 H0 振動擷取韌體/工具鏈的歷史參考（GY-91 韌體、`serial_capture.py`、特徵分析框架仍可沿用，見 WARROOM-2.0-PLAN.md「H1 韌體整合」），但題目描述、接線表、模型選型以外的部分請以新文件為準。
>
