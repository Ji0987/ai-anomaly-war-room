...
見 `README.md` 的接線表與 `BOM.md` 的採購清單(動力/驅動零件已改為全新採購,不拆機械手臂車套件)。重點提醒:TT 馬達由 L298N 獨立電源驅動,**電源地要跟 ESP32-S3 共地**,但 ESP32-S3 不要直接供電給馬達;GY-91 的 I2C 接 GPIO8(SDA)/GPIO9(SCL);「負載」用手壓橡皮擦/氈布即可,不用先做舵機自動化機構。
